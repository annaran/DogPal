...
class ListWalks extends StatefulWidget {
  ListWalks({Key key, this.title}) : super(key: key);
  final String title;

  @override
  _ListWalksState createState() => _ListWalksState();
}

class _ListWalksState extends State<ListWalks> {
  ...
  Future getUserCalendar() async {
    String url = url_loadmywalks + '?id=' + userid;
  ...
  void onCalendarUpdate(String dogid) {
    if (DateTime.parse(selectedDate + ' 23:59:59.000')
      ...
      updateCalendar(dogid);
      refreshData();
      filterCalendardata(selectedDate);
    }
  }

  Future updateCalendar(String dogid) async {
  ...
  void _onTapRow(String timeslotid, String dogid) {
    setState(() {
      selectedTimeslot = timeslotid;
    });

    onCalendarUpdate(dogid);
    refreshData();
  }
  ...
  Widget _buildRow(dynamic item) {
    void _showConfirmationPanel(
        String timeslotid, String question, String dogid) {
      ...
      child: ListTile(
        ...
        onTap: () {
          _showConfirmationPanel(
              item['timeslot_id'], getQuestion(item['status']), item['dog_id']);
